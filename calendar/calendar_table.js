...
function getCoordInfo(){
 ...
 if(navigator.geolocation){
  navigator.geolocation.getCurrentPosition(function(p){
   ...
   if(curLong >= 124 && curLong <= 132){ // 대한민국 좌표범위 (경도)
    if(curLat >= 32.875 && curLat <= 43.125){ // 대한민국 좌표범위 (위도)
     ...
     r.open("GET",`https://api.yukinaserver.net/services/get-location-kr?coord_lat=${curLat}&coord_long=${curLong}`);
     r.send();
    }
   }
